...
\setlrmarginsandblock{50mm}{15mm}{*}

\ExplSyntaxOn
% \dim_set:Nn \marginparwidth { \leftmargin - \rightmargin - \marginparsep }
\dim_set:Nn \marginparwidth { 30mm }
\ExplSyntaxOff

\setmarginnotes{1em}{\marginparwidth}{\baselineskip}
\checkandfixthelayout
\setlength{\evensidemargin}{\oddsidemargin}

\marginparmargin{left}
% \setlength{\sideparmargin}{4cm}
\sideparmargin{left}
...
\makeheadposition{memst}{flushright}{}{}{}
...
\makechapterstyle{memst}{%
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \filldraw[fill=cyan!50!black,line width=0pt] (current page.north east) --
      ++(-100cm, 0cm) -- ++(0cm, -1.7cm) -- ++(100cm, 0cm) -- cycle;

    \filldraw[fill=violet!80!black,line width=0pt] (current page.north east) --
      ++(-1.9cm, 0cm) -- ++(0cm, -7cm) -- ++(1.9cm, 0cm) -- cycle;

    \node [xshift=-0.4cm,yshift=-0.5cm,anchor=north east,text=white] at
      (current page.north east)
    {
      \HUGE \textbf{\thechapter} 
    };

    \node [xshift=-1.5cm,yshift=-1.6cm,rotate=90,anchor=north east,text=white]
...
\setlength\prechapterprecisshift{-2pt}
\aliaspagestyle{chapter}{empty}

\ExplSyntaxOn 
\NewDocumentCommand \completechapterpage { g }
{
  \par
  \vskip20pt
  % \dim_set:Nn \l_tmpa_dim {\marginparwidth + \marginparsep}
  \dim_set:Nn \l_tmpa_dim {3.5cm}
  \hspace*{-\l_tmpa_dim}
